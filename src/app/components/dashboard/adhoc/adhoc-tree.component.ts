...
@Component({
  styleUrls: ['adhoc-tree.css'],
  templateUrl: 'adhoc-tree.html'
})

export class AdHocTreeComponent {
  adHocTree: Array<any>;
  ...
  queryTSS(): void {
    if (!this.isAlreadyLoaded) {
      ...
      this.apiService.getTSTreeAdHoc()
        ...
        try {
          this.adHocTree = scenarios.filter(e => e.OBJ_ID === root_id).map(e => addChildren(e, scenarios));
        } catch (e) {
  ...
  public stateChange(data: Array<PanelBarItemModel>): boolean {
    ...
    this.router.navigate(['/dashboard/ad-hoc'], {queryParams: {filter: filter}});
    return false;
  }
}
