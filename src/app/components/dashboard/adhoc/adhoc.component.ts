import {Component, OnInit, Input, EventEmitter, Output,
  OnChanges, SimpleChanges, forwardRef, AfterViewInit, Inject, ElementRef, AfterViewChecked, ChangeDetectorRef} from '@angular/core';
...
export class AdHocComponent implements OnChanges, OnInit, AfterViewInit, AfterViewChecked {
  static t: AdHocComponent;
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (this.isAlreadyLoaded && this.maskName !== undefined) {
      this.activatedRoute.queryParams.subscribe((params: Params) => {
        ...
        if (typeof p !== 'undefined') {
          ...
          const s = t2.returnUserName() + '_' + this.maskName + '_TS_AdHoc_Selected';
  ...
  ngOnInit() {
    AdHocComponent.t = this;

    const t2 = DashboardHeaderComponent.t;
    const s1 = t2.returnUserName() + '_' + this.maskName + '_PendingRequest';
    localStorage.removeItem(s1);
    const s2 = t2.returnUserName() + '_' + this.maskName + '_TS_AdHoc_Selected';
